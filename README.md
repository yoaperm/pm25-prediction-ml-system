...
Production-ready machine learning system for forecasting PM2.5 air quality using hourly data from Bangkok monitoring stations. It supports both single-value 24-hour-ahead (`T+24h`) forecasts and 24 hourly forecasts generated from a next-hour (`T+1h`) model with automated training, deployment, monitoring, and retraining through Triton Inference Server.
...
- **Predicts PM2.5** exactly 24 hours ahead and as hourly values for the next 24 hours for 5 Bangkok monitoring stations
...
# Optional: train next-hour models used for 24 hourly forecasts
for station in 56 57 58 59 61; do
  docker exec pm25-prediction-ml-system-airflow-scheduler-1 \
    airflow dags trigger pm25_1h_training -c "{\"station_id\": $station}"
  sleep 10
done
...
For the 24-hour pipeline, PM2.5-derived features are shifted to prevent data leakage. For next-hour models, `pm25_lag_1` represents the latest known hourly observation before the target hour.
...
The `pm25_24h_pipeline` DAG runs daily for 24-hour models. Hourly forecasts for the next 24 hours are generated by `pm25_24h_hourly_prediction` once per day and stored separately in PostgreSQL. On the first run, if the hourly prediction table is missing or empty, the DAG backfills daily 24-hour forecast batches for the last 365 days, up to the latest available day. Adjust the `BACKFILL_DAYS` constant in `dags/pm25_1h_prediction_dag.py` if a different window is needed.
...
│   ├── pm25_1h_training_dag.py          # Next-hour model training
│   └── pm25_1h_prediction_dag.py        # Next 24 hourly predictions
├── src/
│   ├── api.py                           # FastAPI service
│   ├── feature_engineering.py           # 19 features with shift(1)
│   ├── hourly_forecast.py               # Shared hourly forecast features
...
│   ├── station_59_24h/
│   ├── station_61_24h/
│   └── station_{id}_1h/                 # Next-hour model artifacts
├── triton_model_repo/
│   ├── pm25_56/
...
│   ├── pm25_59/
│   ├── pm25_61/
│   └── pm25_{station_id}_1h/            # Triton models used recursively
├── results/
│   ├── forecast_24h_results.csv         # Training run history
│   ├── forecast_1h_results.csv          # Next-hour training run history
│   ├── hourly_prediction_batches/        # JSONL batches before DB insert
...
│   ├── predict_1h_stations.py           # T+1h Triton example
