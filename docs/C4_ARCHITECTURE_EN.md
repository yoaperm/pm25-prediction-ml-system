...
A system for forecasting PM2.5 air pollution 24 hours ahead for 5 Bangkok monitoring stations (Stations 56, 57, 58, 59, 61) using hourly data from AirBKK API through an automated ML Pipeline, with auto-retraining when drift is detected.

---

## Level 1: Context Diagram

Shows the system boundary, users, and external systems.

```mermaid
C4Context
    title PM2.5 Prediction ML System — Context Diagram

    Person(user, "End User / Public", "General public who wants to<br/>view PM2.5 forecasts")
    Person(mleng, "ML Engineer / Admin", "Maintains the system,<br/>monitors model performance,<br/>triggers retraining")

    System(pm25sys, "PM2.5 Prediction System", "Forecasts PM2.5<br/>24 hours ahead<br/>for 5 Bangkok stations")

    System_Ext(airbkk, "AirBKK API", "Hourly PM2.5 data<br/>from Thailand's Department<br/>of Pollution Control")
    ...
    Rel(user, pm25sys, "View PM2.5 forecasts", "HTTPS")
    Rel(mleng, pm25sys, "Manage pipeline,<br/>view experiment results", "HTTPS")
    Rel(pm25sys, airbkk, "Fetch hourly<br/>PM2.5 data", "HTTP REST API")
    Rel(github, ec2, "Auto-deploy on push<br/>to main branch", "SSH")
    Rel(mleng, github, "Push code changes", "HTTPS")
```

### Design Decisions — Level 1:
- **AirBKK API as single data source** — Official government data ensures consistency and reliability
- **AWS EC2 deployment** — EC2 required to run Triton Inference Server with sufficient GPU/CPU resources; serverless unsuitable
- **GitHub Actions CI/CD** — Automated deployment on main branch push reduces manual steps

---

## Level 2: Container Diagram

Shows major system components and technologies used.
...
C4Container
    ...
    Container_Boundary(pm25sys, "PM2.5 Prediction System") {
        ...
        Container(streamlit, "Streamlit Dashboard", "Python, Streamlit 1.41", "Web UI for predictions,<br/>model comparison,<br/>monitoring dashboard")
        Container(api, "FastAPI Service", "Python, FastAPI 0.115", "REST API for inference<br/>/predict, /actual, /retrain<br/>X-API-Key authentication")
        Container(triton, "Triton Inference Server", "NVIDIA Triton 24.08, ONNX Runtime", "High-performance inference<br/>5-10ms latency, auto-reload<br/>every 30 seconds")
    ...
    Rel(user, streamlit, "View forecasts", "HTTPS :8501")
    Rel(mleng, airflow, "Manage pipelines", "HTTPS :8080")
    Rel(mleng, mlflow, "View experiments", "HTTPS :5001")
    Rel(streamlit, api, "Call /predict", "HTTP :8000")
    Rel(api, triton, "Inference request", "HTTP/gRPC :8000")
    Rel(api, models, "Load active_model.json<br/>& feature_columns.json", "File I/O")
    Rel(triton, models, "Load ONNX models<br/>poll every 30s", "File I/O")
...
- **Triton Inference Server separate from FastAPI** — Triton manages batching, concurrency, model versioning better than direct onnxruntime; FastAPI acts as API gateway + feature engineering
- **ONNX-only deployment** — All models (sklearn, XGBoost, PyTorch LSTM) exported to ONNX for framework-agnostic inference
- **PostgreSQL shared instance** — Airflow metadata, MLflow backend, and time-series data in single DB (separate schemas) reduces operational overhead
- **File-based model repository** — File system used instead of model registry because Triton polls directory directly; active_model.json points to current ONNX file
- **MLflow for experiment tracking** — MLflow Model Registry not used; ONNX + Triton path used instead

---

## Level 3: Component Diagram (ML Pipeline Container)

Shows internal structure of ML pipeline orchestrated by Airflow.
...
C4Component
    ...
    Container_Boundary(ml_pipeline, "ML Pipeline (Airflow DAGs + src/)") {
        ...
        Component(ingest, "Hourly Ingest", "pm25_hourly_ingest_dag.py<br/>+ airbkk_client.py + airflow_db.py", "Fetch hourly PM2.5<br/>from AirBKK API<br/>validate & store to DB")

        Component(data_loader, "Data Loader", "src/data_loader.py", "Query data from PostgreSQL<br/>or load from CSV/Excel<br/>split train/val/test")
        ...
        Component(feat_eng, "Feature Engineering", "src/feature_engineering.py", "Create 19 features:<br/>Lags [1,2,3,6,12,24h]<br/>Rolling mean/std [6,12,24h]<br/>Diff [1h,24h], Time features<br/>use shift(1) to prevent leakage")
        ...
        Component(evaluator, "Evaluator", "src/evaluate.py", "Compute MAE, RMSE, R²<br/>compare new vs production<br/>deploy if MAE improves")
        ...
        Component(publisher, "Triton Publisher", "src/triton_utils.py", "Generate config.pbtxt<br/>copy ONNX → triton_model_repo<br/>Triton auto-reload every 30s")

        Component(monitor, "Drift Monitor", "src/monitor.py<br/>+ src/airflow_monitor.py", "Rolling 14-day RMSE<br/>PSI drift detection<br/>Sensor quality checks<br/>Trigger retrain if degraded")
    }
...
- **5 competing models** — Linear, Ridge, RF, XGBoost, LSTM all trained each cycle; model with lowest MAE on test set deployed; makes system adaptive to data pattern changes
- **TimeSeriesSplit(n_splits=3)** — Used instead of random k-fold to preserve temporal order and prevent data leakage
- **shift(1) on all features** — Critical design choice ensures features don't use information from prediction hour
- **PSI (Population Stability Index)** — Measures distribution shift between predicted and actual; PSI > 0.2 = significant drift → trigger retrain
- **ONNX export per model type** — Different export paths per framework: sklearn uses skl2onnx, XGBoost uses onnxmltools, LSTM uses torch.onnx.export
- **Versioned ONNX files** — Old models kept; filename includes date range `{model}_{train_start}_{train_end}.onnx` for rollback capability
...
sequenceDiagram
    ...
    User->>Streamlit: Open Dashboard, Login
    Streamlit->>Streamlit: Session-based Auth

    User->>Streamlit: Enter PM2.5 history (≥15 days)
    ...
    Streamlit-->>User: Display forecast + Air Quality Level
```
...
| Triton Server | Low latency (5-10ms), auto-batching | Adds operational complexity, requires model repo management |
| 5 models compete | Adaptive — picks best model for current data | Training takes 15-20 hours per 5 stations |
| PostgreSQL shared | Reduces ops overhead, single backup point | Single point of failure if DB goes down |
| shift(1) on features | 100% leakage prevention | Loses 1 row per feature |
| PSI + RMSE monitoring | Catches both drift and accuracy drop | Requires matched actual data to calculate |
| Docker Compose | Simple, reproducible, one-command deployment | No horizontal scaling like Kubernetes |
| File-based model versioning | Simple, Triton polls directly | No registry UI, manual rollback process |
